...
from pente_engine import (
    check_win_pente, detect_capture_pente, apply_capture_pente,
    check_win_by_capture, get_legal_moves_proximity_pente, simulate_game_pente
)
...
class Player:
    ...
    def __init__(self, rules='gomoku', board_size=15):
        ...
        # Pente: rastreamento de capturas
        self.captures_p1 = 0
        self.captures_p2 = 0
    ...
    def play(self, board, turn_number, last_opponent_move):
        ...
        # Pente: atualiza capturas após jogada
        if self.rules == 'pente':
            temp_board = board.copy()
            temp_board[best_move] = self.player_id
            captured = detect_capture_pente(temp_board, best_move[0], best_move[1], self.board_size)
            if self.player_id == 1:
                self.captures_p1 += captured
            else:
                self.captures_p2 += captured
            
            print(f"Turn {turn_number}: {best_move} "
                  f"(iter={iterations}, time={elapsed:.2f}s, visits={visits}, value={value:.3f}, "
                  f"captures={self.captures_p1 if self.player_id == 1 else self.captures_p2})")
        else:
            print(f"Turn {turn_number}: {best_move} "
                  f"(iter={iterations}, time={elapsed:.2f}s, visits={visits}, value={value:.3f})")
    ...
    def _mcts_iteration(self, board, player):
        """Uma iteração MCTS (suporta Gomoku e Pente)"""
        node = self.root
        temp_board = board.copy()
        temp_player = player
        search_path = [node]
        
        # Pente: copia estado de capturas
        temp_captures_p1 = self.captures_p1
        temp_captures_p2 = self.captures_p2
        ...
        while node.children:
            ...
            # Pente: aplica capturas
            if self.rules == 'pente':
                captured = apply_capture_pente(temp_board, move[0], move[1], self.board_size)
                if temp_player == 1:
        ...
        # Verifica vitória
        if self.rules == 'pente':
            # Pente: verifica vitória por captura primeiro
            winner = check_win_by_capture(temp_captures_p1, temp_captures_p2)
            if winner == 0:
                winner = check_win_pente(temp_board, self.board_size)
        else:
            # Gomoku
        ...
        if winner == 0:
            ...
            if self.rules == 'pente':
                legal_moves = get_legal_moves_proximity_pente(temp_board, self.board_size, distance=2)
            else:
